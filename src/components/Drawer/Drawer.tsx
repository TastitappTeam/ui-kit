...
const drawerContentVariants = cva(
  'fixed z-50 flex h-auto flex-col border bg-white',
  {
    variants: {
      direction: {
...
const drawerBarVariants = cva('h-2 w-[100px] rounded-full bg-gray-100', {
  variants: {
    direction: {
      right: 'my-auto ml-4',
      top: 'mx-auto mb-4',
...
>(({ className, children, ...props }, ref) => {
  ...
  const isDirectionBottom = direction === 'bottom';
    ...
    <DrawerPortal>
      ...
      >
        {isDirectionTop ? (
          <>
            {children}
            <div className={drawerBarVariants({ direction })} />
          </>
        ) : isDirectionBottom ? (
          <>
            <div className={drawerBarVariants({ direction })} />
